...
export class AccountEndpoint extends EndpointBase {
  ...
  get articleUrl() { return this.configurations.baseUrl + '/api/Articles/'; }
  ...
  getArticles<T>(): Observable<T> {
    const endpointUrl =  this.articleUrl;

    return this.http.get<T>(endpointUrl, this.requestHeaders).pipe<T>(
      catchError(error => {
        return this.handleError(error, () => this.getArticles());
      }));
  }
