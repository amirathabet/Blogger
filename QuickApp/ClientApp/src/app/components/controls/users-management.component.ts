...
export class UsersManagementComponent implements OnInit, AfterViewInit {
  ...
  loadData() {
    ...
    if (this.canViewRoles) {
      this.accountService.getUsersAndRoles().subscribe(results => {
      ...
      this.accountService.getUsers().subscribe(users => {
        ...
        console.log("this.onDataLoadSuccessful", this.onDataLoadSuccessful(users, this.accountService.currentUser.roles.map(x => new Role(x))));
      }, error => this.onDataLoadFailed(error));
    }
  }
